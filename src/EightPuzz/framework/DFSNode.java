...
public class DFSNode{
	 ...
	 * Class constructor.
	 */
	public DFSNode(DFSNode parent, Action action, int[][] state)  {
		this.parent = parent;
		this.action = action;
	 ...
	 * except the root.
	 */
	public DFSNode(int[] input) throws Exception{
	 ...
	 */
	public List<DFSNode> getChildren() throws Exception {
		...
		int val;
		List<DFSNode> children = new LinkedList<DFSNode>();
		...
		if((x < BOARD_SIZE - 1) && (x >= 0) && (y < BOARD_SIZE) && (y >= 0)){ 
			...
			DFSNode up = new DFSNode(this, upAction, upTiles);
			children.add(up);
		}
		...
		if((x < BOARD_SIZE) && (x > 0) && (y < BOARD_SIZE) && (y >= 0)){
			...
			DFSNode down = new DFSNode(this, downAction, downTiles);
			children.add(down);			
		}
		...
		if((x < BOARD_SIZE) && (x >= 0 ) && (y < BOARD_SIZE - 1) && (y >= 0)){
			...
			DFSNode left = new DFSNode(this, leftAction, leftTiles);
			children.add(left);
		}
		...
		if((x < BOARD_SIZE) && (x >= 0 ) && (y < BOARD_SIZE) && (y > 0)){
			...
			DFSNode right = new DFSNode(this, rightAction, rightTiles);
			children.add(right);
		}
